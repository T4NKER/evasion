...
def increment_integer_to_100001(filename):
    """Append code to increment an integer to 100001 in the given file."""
    increment_code = """
for i in range(100001):
    pass
"""
    with open(filename, 'a') as f:
        f.write(increment_code)
...
def delayed_execution(filename):
    """Delay execution by 101 seconds, ensure 101 seconds have passed, then decrypt and execute the program."""
    ...
    if elapsed_time >= 101:
        print("101 seconds have passed. Decrypting and executing the program.")
        print("File contents after decryption:")
        decrypted_data = decrypt_file(filename)
        # Print the decrypted data as text (assuming it's a Python file)
        print(decrypted_data.decode('utf-8'))
        ...
        print("101 seconds have not passed. The program will not be decrypted or executed.")
...
def select_program_file():
    ...
    if filename:
        encrypted_filename = encrypt_file(filename)
        increased_filename = increase_file_size(encrypted_filename, 101)
        incremented_filename = increment_integer_to_100001(increased_filename)

        # Start the delayed execution in a separate thread to keep the GUI responsive
        thread = threading.Thread(target=delayed_execution, args=(incremented_filename,))
        thread.start()
        root.destroy()
